...
use crate::errors::*;
use crate::utilities::discretesampling::{
    adjust_eta, is_multiple_of, lazy_threshold, conditional_lazy_threshold, sample_within_bounds,
};
...
pub fn sparse_vector<R: ThreadRandGen + Copy>(
    eta1: Eta,
    eta2: Eta,
    c: usize,
...
/// Care should be taken given that this mechanism allows for an arbitrary
/// number of queries which may amplify timing channels.
pub fn sparse_vector_with_gap<R: ThreadRandGen + Copy>(
    eta1: Eta,
    eta2: Eta,
    c: usize,
    gaps: &Vec<f64>,
    ...
    for i in 0..gaps.len() {
        if !is_multiple_of(&arithmetic_config.get_float(gaps[i]), &g) {
            return Err("All gaps must be integer multiples of gamma.".into());
        }
        if gaps[i] < 0.0 {
            return Err("All gaps must be non-negative.".into());
        }
    }
    ...
    for i in 0..query_values.len() {
        ...
        if a.is_infinite() && a.is_sign_positive() {
            ...
            for i in 0..gaps.len(){
                ...
                let ai = conditional_lazy_threshold(eta2, & mut arithmetic_config, &g_inv, &thresh, &conditional_thresh, rng, optimize)?;
                ...
                else { // quit with the current gap
                    break;
                }
            }
...
mod tests {
    ...
    fn test_sparse_vector_with_gap() {
        ...
        let outputs = sparse_vector_with_gap(
            eta1, eta2, c, &gaps, &queries, gamma, q_min, q_max, w, rng, optimize,
        );
        println!("{:?}",outputs);
    ...
    fn test_sparse_vector() {
        ...
        let outputs = sparse_vector(
            eta1, eta2, c, &queries, gamma, q_min, q_max, w, rng, optimize,
        );
        assert!(outputs.is_ok());
        let outputs = outputs.unwrap();
        assert!(outputs.len() >= c); // Should always pass.
