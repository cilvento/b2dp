...
impl Default for PartitionBoundOptions {
    ...
    { 
        ...
        PartitionBoundOptions { exp_options, sparsity_control: None, sparsity_padding: 1.5, estimate_padding: 1.5 } 
    }
}
...
impl PartitionBound {
    ...
    fn get_upper_bound(d: usize, i: usize, x: &Vec<i64>) -> i64 {
        ...
        let mut t = 0;
        if (a as usize) < i { t = i-(a as usize); }

        let mut x_temp: Vec<i64> = Vec::new();
        for j in t..i+1 {
            if j < x.len() { x_temp.push(x[j]); }
            else { x_temp.push(0); }
        }
        ...
        while a > 0 {
            j = k;
            while /*j >= 0 &&*/ x_temp[j] <= x_temp[k] && a >= 0 {
                x_temp[j] += 1;
                a -= 1;
                if j > 0 { j -= 1; }
                else { break; } 
        ...
        if a < 0 { return x_temp[k]-1; }
        else { return x_temp[k]; }
    }
    ...
    fn get_lower_bound(d: usize, i: usize, x: &Vec<i64>) -> i64 {
        ...
        if r < 0 || x_temp[k] < 0 { return x_temp[k]+1; }
        else { return x_temp[k]; }
    }
        ...
        -> Result<PartitionBound, &'static str> 
    {
        ...
        let pb = PartitionBound { upper:upper,
                                  lower:lower, 
                                  count: total_count, 
                                  cells: total_cells, 
        ...
        -> Result<PartitionBound, &'static str>
    {
        ...
        if let Some(sparsity_budget) = options.sparsity_control {
            ...
            let mut noisy_non_zeros = clamped_laplace_mechanism(sparsity_budget, 0.0, cells as f64, non_zeros as f64,
                        1.0, rng, options.exp_options).unwrap();
            ...
            if noisy_non_zeros < cells { cells = noisy_non_zeros; }
        }
        ...
        for i in 0..cells {
            ...
            let gamma = 1.0; 
            let noisy_estimate = clamped_laplace_mechanism(eta, 0.0, total_count as f64, xi as f64,
                gamma, rng, options.exp_options).unwrap();
        ...
        for i in 0..upper_bound.len() {
            ...
            if lower < pb_baseline.lower[i] { lower = pb_baseline.lower[i]; }
        ...
        let pb = PartitionBound { upper: upper_bound, 
                                  lower: lower_bound, 
                                  count: total_count, 
                                  cells: cells, 
...
mod tests {
    use super::*;
    use crate::{Eta, GeneratorOpenSSL};
    #[test]
    fn test_from_dist() {
    ...
    fn test_from_noisy_estimates() {
        ...
        let rng = GeneratorOpenSSL {};
        let options: PartitionBoundOptions = Default::default();
        let pb = PartitionBound::from_noisy_estimates(total_count as usize, Some(total_cells), 
            &x, eta, rng, options).unwrap();
    ...
    fn test_from_noisy_estimates_with_sparsity() {
        ...
        let total_count: i64 = x.iter().sum();
        let rng = GeneratorOpenSSL {};
        let options = PartitionBoundOptions { sparsity_control: Some(sparsity_eta), ..Default::default()};
        let pb = PartitionBound::from_noisy_estimates(total_count as usize, Some(total_cells), 
            &x, eta, rng, options).unwrap();
        let baseline_pb = PartitionBound::with_cells(total_count as usize, total_cells).unwrap();
